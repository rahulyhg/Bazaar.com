...
$s = $_POST["search"];
 $conn = oci_connect($username = 'ns3',
        ...
        if($_POST["search"])
        $sql1 = "select * from inventory where item LIKE '%$s%' OR category='$s' OR sub_category='$s'";
        $stid1 = oci_parse($conn, $sql1);
        oci_execute($stid1);
        $sql2 = "select distinct category from inventory ";
...
{
$k=$_GET['c'];
$sql3 = "select distinct sub_category from inventory where category='$k'";
...
<body>
	...
	<div class="row">
                                  ...
                                  <div class="form-group">
									...
									<select class="form-control" id="sub_category" >
                                        ...
                                        <?php
                                          ...
                                          {  
                                          $n=oci_result($stid3, 'SUB_CATEGORY') ;
                                          echo '<option value="'.$n.'">'.$n.'</option>';
                                         };
                                         ?>
                                    </select>
        ...
        if($_POST["search"])
        {
         $sql3 = "select count(*) as cnt from inventory where item LIKE '%$s%' OR category='$s' OR sub_category='$s'";
        $stid3 = oci_parse($conn, $sql3);
        oci_execute($stid3);
        if(oci_fetch($stid3))
    ...
    <section class="col-xs-12 col-sm-6 col-md-12">
        <?php
          while(oci_fetch($stid1))
          {
            ...
            $cat=oci_result($stid1, 'CATEGORY') ;
            $u=oci_result($stid1, 'UNIT_PRICE') ;
            $n=oci_result($stid1, 'ITEM') ;
            $l=oci_result($stid1, 'ID') ;

             $sql2 = "select rating from rating, inventory where inventory.id = rating.inventory_id AND inventory.item='$n'";
        $stid2 = oci_parse($conn, $sql2);
        oci_execute($stid2);
        if(oci_fetch($stid2))
        $r=oci_result($stid2, 'RATING') ;
        ?>
